...
static const char* state_to_string(int state) {
    if (state == 0) return "zoo_closed_state";
    if (state == ZOO_CONNECTING_STATE) return "zoo_state_connecting";
    if (state == ZOO_ASSOCIATING_STATE) return "zoo_state_associating";
    if (state == ZOO_CONNECTED_STATE) return "zoo_state_connected";
    if (state == ZOO_EXPIRED_SESSION_STATE) return "zoo_state_expired_session";
    if (state == ZOO_AUTH_FAILED_STATE) return "zoo_state_auth_failed";

    return "zoo_state_invalid";
}

static const char* type_to_string(int state) {
    if (state == ZOO_CREATED_EVENT) return "zoo_event_created";
    if (state == ZOO_DELETED_EVENT) return "zoo_event_deleted";
    if (state == ZOO_CHANGED_EVENT) return "zoo_event_changed";
    if (state == ZOO_CHILD_EVENT) return "zoo_event_child";
    if (state == ZOO_SESSION_EVENT) return "zoo_event_session";
    if (state == ZOO_NOTWATCHING_EVENT) return "zoo_event_notwatching";

    return "zoo_event_unknow";
}

static void zookeeper_watch_func(zhandle_t* zh, int type, int state, const char* path,
                                 void* watcherCtx) {
    ...
    printf("zk_watcher, type[%d:%s] state[%d:%s] path[%s]\n", type, type_to_string(type),
           state, state_to_string(state), path_str.c_str());
...
const char* zk_cpp::error_string(int32_t rc) { return zerror(rc); }

const char* zk_cpp::state_to_string(int32_t state) {
    return details::state_to_string(state);
}

void zk_cpp::set_log_lvl(zoo_log_lvl lvl) { zoo_set_debug_level((ZooLogLevel)lvl); }

void zk_cpp::set_log_stream(FILE* file) { zoo_set_log_stream(file); }
...
zoo_acl_t zk_cpp::create_digest_acl(int32_t perms, const std::string& user,
...
zoo_rc zk_cpp::connect(const std::string& url) {
    ...
    m_zh = zookeeper_init(url.c_str(), details::zookeeper_watch_func, zoo_recv_time_out,
                          nullptr, this, 0);
...
zoo_rc zk_cpp::create_node(const char* path, const std::string& value,
                           const std::vector<zoo_acl_t>& acl, int32_t create_flags,
    ...
    zoo_rc rt =
        (zoo_rc)zoo_create((zhandle_t*)m_zh, path, value.c_str(), (int)value.size(),
                           &acl_v, create_flags, path_buffer, (int)path_buffer_len);
...
zoo_rc zk_cpp::create_persistent_node(const char* path, const std::string& value,
                                      const std::vector<zoo_acl_t>& acl) {
    return create_node(path, value, acl, 0, nullptr, 0);
}

zoo_rc zk_cpp::create_sequence_node(const char* path, const std::string& value,
                                    const std::vector<zoo_acl_t>& acl,
                                    std::string& returned_path_name) {
    char rpath[zoo_path_buf_len] = {0};
    zoo_rc rt =
        create_node(path, value, acl, ZOO_SEQUENCE, rpath, (int32_t)sizeof(rpath));
    returned_path_name = rpath;
    return rt;
}

zoo_rc zk_cpp::create_ephemeral_node(const char* path, const std::string& value,
                                     const std::vector<zoo_acl_t>& acl) {
    return create_node(path, value, acl, ZOO_EPHEMERAL, nullptr, 0);
}

zoo_rc zk_cpp::create_sequance_ephemeral_node(const char* path, const std::string& value,
                                              const std::vector<zoo_acl_t>& acl,
                                              std::string& returned_path_name) {
    char rpath[zoo_path_buf_len] = {0};
    zoo_rc rt = create_node(path, value, acl, ZOO_SEQUENCE | ZOO_EPHEMERAL, rpath,
...
zoo_rc zk_cpp::get_node(const char* path, std::string& out_value, zoo_state_t* info,
                        bool watch) {
    struct Stat s = {0};
    ...
    zoo_rc rt =
        (zoo_rc)zoo_get((zhandle_t*)m_zh, path, watch, data_buffer, &buf_size, &s);
    do {
        if (rt != z_ok) {
            break;
        }
        ...
        if (s.dataLength > buf_size) {
            ...
            rt = (zoo_rc)zoo_get((zhandle_t*)m_zh, path, watch, data_buffer, &buf_size,
                                 &s);
            if (rt != z_ok) {
                break;
            }
...
zoo_rc zk_cpp::set_node(const char* path, const std::string& value, int32_t version) {
    return (zoo_rc)zoo_set((zhandle_t*)m_zh, path, value.c_str(), (int)value.size(),
                           version);
}

zoo_rc zk_cpp::get_children(const char* path, std::vector<std::string>& children,
                            bool watch) {
...
zoo_rc zk_cpp::set_acl(const char* path, const std::vector<zoo_acl_t>& acl,
                       int32_t version) {
    struct ACL_vector acl_v;
    acl_v.count = (int32_t)acl.size();
    struct ACL* acl_list = NULL;
...
zoo_rc zk_cpp::add_auth(const std::string& user_name, const std::string& user_passwd) {
    ...
    return (zoo_rc)zoo_add_auth((zhandle_t*)m_zh, "digest", cert.c_str(),
                                (int)cert.size(), details::default_void_completion_func,
                                this);
}

zoo_rc zk_cpp::watch_data_change(const char* path,
                                 const data_change_event_handler_t& handler,
                                 std::string* value) {
...
zoo_rc zk_cpp::watch_children_event(const char* path,
                                    const child_event_handler_t& handler,
...
void zk_cpp::add_data_event_handler(const std::string& path,
                                    zk_cpp::data_event_handler_ptr handler) {
    std::lock_guard<std::mutex> locker(m_mtx);

    m_data_event_map[path] = handler;
}

void zk_cpp::add_child_event_handler(const std::string& path,
                                     zk_cpp::child_event_handler_ptr handler) {
    std::lock_guard<std::mutex> locker(m_mtx);
...
void zk_cpp::on_session_connected() { printf("zk_cpp::on_session_connected\n"); }
